...
{
  template <typename T>
  int BinaryTreeSet<T>::findHeightRecursive(const BinaryNode<T> *node) const
  {
    if (!node)
    {
      return -1;
  ...
  template <typename T>
  int BinaryTreeSet<T>::height() const
  {
  ...
  template <typename T>
  void BinaryTreeSet<T>::insert(const T &value)
  {
    bool inserted = false;
  ...
  template <typename T>
  void BinaryTreeSet<T>::insertRange(const std::vector<T> &range)
  {
  ...
  template <typename T>
  void BinaryTreeSet<T>::merge(const BinaryTreeSet<T> &set)
  {
    set.traverseInorder([this](const T &value)
                        { this->insert(value); });
  }

  template <typename T>
  bool BinaryTreeSet<T>::contains(const T &value) const
  {
  ...
  template <typename T>
  BinaryNode<T> *BinaryTreeSet<T>::find(const T &value) const
  {
  ...
  template <typename T>
  bool BinaryTreeSet<T>::erase(const T &value)
  {
    bool removed = false;
  ...
  template <typename T>
  void BinaryTreeSet<T>::clearTreeRecursive(BinaryNode<T> *node)
  {
    if (node == nullptr)
    {
      return;
  ...
  template <typename T>
  void BinaryTreeSet<T>::traverseInorderRecursive(const BinaryNode<T> *node, std::function<void(const T &)> callback) const
  {
    if (!node)
    {
      return;
  ...
  template <typename T>
  void BinaryTreeSet<T>::traverseInorder(std::function<void(const T &)> callback) const
  {
    traverseInorderRecursive(root, callback);
  }

  template <typename T>
  void BinaryTreeSet<T>::traversePreorderRecursive(const BinaryNode<T> *node, std::function<void(const T &)> callback) const
  {
    if (!node)
    {
      return;
  ...
  template <typename T>
  void BinaryTreeSet<T>::traversePreorder(std::function<void(const T &)> callback) const
  {
    traversePreorderRecursive(root, callback);
  }

  template <typename T>
  void BinaryTreeSet<T>::traversePostorderRecursive(const BinaryNode<T> *node, std::function<void(const T &)> callback) const
  {
    if (!node)
    {
      return;
  ...
  template <typename T>
  void BinaryTreeSet<T>::traversePostorder(std::function<void(const T &)> callback) const
  {
